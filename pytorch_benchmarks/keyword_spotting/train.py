...
def get_default_optimizer(net: nn.Module) -> optim.Optimizer:
    return optim.Adam(net.parameters(), lr=0.0005, weight_decay=1e-4)
...
def get_default_scheduler(opt: optim.Optimizer) -> optim.lr_scheduler._LRScheduler:
    return optim.lr_scheduler.MultiStepLR(opt, milestones=[12, 24], gamma=0.2)
...
def train_one_epoch(
    ...
    with tqdm(total=len(train), unit="batch") as tepoch:
        ...
        for audio, target in train:
            ...
            output, loss = _run_model(model, audio, target, criterion, device)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
