...
def get_data(dataset: Literal['cifar10', 'tiny-imagenet', 'imagenet'],
    ...
    if dataset == 'cifar10':
        return get_cifar10(data_dir, download, validation_split,
                           train_transform, test_transform)
    elif dataset == 'tiny-imagenet':
        return get_tiny_imagenet(data_dir, download, validation_split,
                                 train_transform, test_transform)
        ...
        msg = (f"Unknown dataset: {dataset}. "
               "Please choose between: cifar10, tiny-imagenet, imagenet")
        raise ValueError(msg)


def get_train_val_datasets(train_set: torch.utils.data.Dataset,
...
def get_cifar10(data_dir: str, download: bool, validation_split: float,
                train_transform, test_transform):
...
def get_tiny_imagenet(data_dir: str, download: bool, validation_split: float,
                      train_transform, test_transform):
    URL = 'http://cs231n.stanford.edu/tiny-imagenet-200.zip'
    # check if dataset is already downloaded
    is_downloaded = (os.path.exists(os.path.join(data_dir, 'tiny-imagenet-200.zip')) and
                     os.path.exists(os.path.join(data_dir, 'tiny-imagenet-200')))
    msg = ("Dataset is not downloaded or not decompressed, "
           "please set download=True or decompress the dataset manually")
    assert download or is_downloaded, msg
    if download and not is_downloaded:
        ds_utils.download_and_extract_archive(URL, data_dir)
        msg = "Downloaded dataset does not match the expected checksum"
        assert ds_utils.check_integrity(os.path.join(data_dir, 'tiny-imagenet-200.zip'),
                                        '90528d7ca1a48142e341f4ef8d21d0de'), msg
    train_dataset = datasets.ImageFolder(root=os.path.join(data_dir, 'tiny-imagenet-200', 'train'),
                                         transform=train_transform)
    val_dataset = datasets.ImageFolder(root=os.path.join(data_dir, 'tiny-imagenet-200', 'train'),
...
def get_imagenet(data_dir: str, validation_split: float, train_transform, test_transform):
    train_dataset = datasets.ImageNet(data_dir, split='train', transform=train_transform)
    val_dataset = datasets.ImageNet(data_dir, split='val', transform=test_transform)
    test_dataset = datasets.ImageNet(data_dir, split='val', transform=test_transform)
    return *get_train_val_datasets(train_dataset, val_dataset, validation_split), test_dataset


def build_dataloaders(datasets: tuple,
                      batch_size: int = 32,
                      num_workers: int = 2,
                      seed=None):
